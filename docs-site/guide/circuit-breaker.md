...
```typescript
// Internal wallet flow (simplified):
...
if (this.circuitBreaker) {
  ...
  // "ALLOW" → counter = 0
  // "DENY"  → counter++, if counter >= threshold → open circuit
  // "PENDING" → no change
}
```
...
// Custom circuit breaker: trip after 3 denials, 60-second cooldown
...
// result.data.circuitBreaker = { threshold: 3, cooldownMs: 60000, isOpen: false }
```
